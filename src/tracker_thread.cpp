...
void tracker_thread(
  ...
  while (true) {
    ...
    //FILE 	      *outfile_rssi;	
    //char* 	       filename_rssi;
    //filename_rssi  = (char*)calloc(255, sizeof(char));
    //sprintf(filename_rssi, "LTE_RSSI_ID%d_RB6.dat", tracked_cell.n_id_cell);
	//outfile_rssi = fopen(filename_rssi,"ab+");  
    //cvec sym_raw=syms(itpp_ext::matlab_range(0,1,71));
    //float sfn = 0;
	//float sym_real = 0;
	//float sym_imag = 0;
	//float rssi = 0;
	//fwrite(&sfn,sizeof(sfn),1,outfile_rssi);
    //for( int a=0; a < sym_raw.size(); a++){
		    //sym_real = static_cast<float>(real(sym_raw(a)));
		    //sym_imag = static_cast<float>(imag(sym_raw(a)));		    
			//rssi += sym_real*sym_real + sym_imag*sym_imag;
    //}
    //rssi = rssi/72;   
    //fwrite(&rssi,sizeof(rssi),1,outfile_rssi);
    //fclose(outfile_rssi);
	////////////////////////////
    ...
    for (uint8 port_num=0;port_num<tracked_cell.n_ports;port_num++) {
      ...
      char* 	       filename_csi;
      //char* 	       filename_rsrp;
	  filename_csi   = (char*)calloc(255, sizeof(char));
	  //filename_rsrp  = (char*)calloc(255, sizeof(char));
      sprintf(filename_csi, "LTE_CSI_ID%d_RB6_TX%d.dat", tracked_cell.n_id_cell,port_num+1);
	  outfile_csi = fopen(filename_csi,"ab+");
	  //sprintf(filename_rsrp, "LTE_RSRP_ID%d_RB6_TX%d.dat", tracked_cell.n_id_cell,port_num+1);
	  //outfile_rsrp = fopen(filename_rsrp,"ab+");
  	  	
	  float sfn = 0;
	  float ce_real = 0;
	  float ce_imag = 0;
	  //float rsrp = 0;
	  fwrite(&sfn,sizeof(sfn),1,outfile_csi);
	  //fwrite(&sfn,sizeof(sfn),1,outfile_rsrp);
      for( int a=0; a < ce_raw.size(); a++){
		    ce_real = static_cast<float>(real(ce_raw(a)));
		    ce_imag = static_cast<float>(imag(ce_raw(a)));	    	    
            fwrite(&ce_real,sizeof(ce_real),1,outfile_csi);
			fwrite(&ce_imag,sizeof(ce_imag),1,outfile_csi);			
			//rsrp += ce_real*ce_real + ce_imag*ce_imag;
      }  
      fclose(outfile_csi);   
      //rsrp = rsrp/12;
      //fwrite(&rsrp,sizeof(rsrp),1,outfile_rsrp);
	  //fclose(outfile_rsrp);
	  //////////////////////////
